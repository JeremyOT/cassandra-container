...
RUN apt-get update && apt-get install procps libsnappy-dev curl libjna-java python-yaml -y; \
    ...
    curl -L http://apache.claz.org/cassandra/2.0.16/apache-cassandra-2.0.16-bin.tar.gz > /tmp/cassandra/apache-cassandra-2.0.16-bin.tar.gz; \
    cd /tmp/cassandra; \
    tar xzvf apache-cassandra-2.0.16-bin.tar.gz; \
    cp -r apache-cassandra-2.0.16/* /usr/lib/cassandra; \
